...
function startGame(){
  ...
  caughtText.textContent = `${gameCaught}`
  softMessage(gameMsg, "Tangkap 10 kupu-kupu. Nggak ada tombol skip buat hidup.", "neutral")
...
function loseGame(){
  gameRunning = false
  clearGame()
  softMessage(gameMsg, "Gagal. Ulang dari awal ya. Kupu-kupunya juga capek dikejar.", "bad")
...
playBtn.addEventListener("click", async ()=>{
  try{
    ...
    softMessage(document.querySelector('[data-slide="1"] .hint'), "Audio diblokir. Tap Play lagi, atau naikin volume, atau salaman sama browser.", "bad")
  }
})
...
function extinguishCake(reason){
  ...
  micStatus.textContent = reason === "mic" ? "Nice. Lilinnya mati karena tiupan." : "Oke. Lilinnya mati karena tap."
...
async function requestMic(){
  ...
  try{
    ...
    micStatus.textContent = "Izin mic ditolak / nggak tersedia. Pakai opsi tap kuenya aja."
...
function animateLetter(){
  ...
  spans.forEach(n=> letterScroller.appendChild(n))
  ...
  letterInterval = setInterval(()=>{
    ...
    idx++
    const nearBottom = (letterScroller.scrollHeight - (letterScroller.scrollTop + letterScroller.clientHeight)) < 120
    if(nearBottom){
